    ...
    def get_weather(self):
        ...
        if response.status_code == 200:
            ...
            try:
                ...
                dawn_time = datetime.strptime(dawn, '%I:%M %p')
                current_time = datetime.now()
            except ValueError:
                return "Error: Unable to parse sunrise/sunset times."

            # Check if current time is after sunset but before dawn (nighttime)
            if sunset_time < current_time < dawn_time:
                is_night = True
            else:
                is_night = False
            ...
            # Check if it's night time and if the condition is "Clear" or "Sunny"
            if is_night and condition.lower() in ["sunny", "clear", "clear sky"]:
                selected_emoji = "🌙"  # Change to moon emoji at night
