...
class WxFetcher:
    ...
    def get_weather(self):
        ...
        if response.status_code == 200:
            ...
            # Convert the sunrise and sunset times to 24-hour format for comparison
            sunset_time = datetime.strptime(sunset, '%I:%M %p').time()
            sunrise_time = datetime.strptime(dawn, '%I:%M %p').time()
            current_time = datetime.now().time()

            # Check if the current time is between sunset and sunrise (night time)
            if current_time > sunset_time or current_time < sunrise_time:
                # If it's night time, replace the "Sunny", "Clear", "Clear sky" condition with a moon emoji
                if any(condition.lower() in x.lower() for x in ["Sunny", "Clear", "Clear sky"]):
                    condition = "Night"  # Change condition to "Night"
            ...
            emojis = {
                ...
                "🌬️": ["Windy"],
                "🌫️": ["Fog", "Mist"],
                "🌙": ["Night", "Clear sky", "Clear"],  # Moon emoji for night time
            }
