...
class PollenLevels():
    def __init__(self, locations):
        # Modify the API to request US AQI (use us_aqi instead of european_aqi)
        self.pollen_api = "https://air-quality-api.open-meteo.com/v1/air-quality?latitude={0}&longitude={1}&current=alder_pollen,birch_pollen,grass_pollen,mugwort_pollen,ragweed_pollen,us_aqi"
    ...
    def get_aqi_description(self, aqi_value):
        """Return the AQI description based on the US Air Quality Index."""
        ...
        for lonlat in self.locations:
            try:
                ...
                with urlopen(self.pollen_api.format(latitude, longitude)) as Client:
                    ...
                    if "current" in data:
                        # Check for us_aqi and add it to the top of the output
                        if "us_aqi" in data["current"]:
                            aqi_value = data["current"]["us_aqi"]
                            aqi_description = self.get_aqi_description(aqi_value)
                            lonlatResponse.append("US AQI: {0} ({1})\n".format(str(aqi_value), aqi_description))

                        # Add the pollen levels below the US AQI
                        ...
                        print("Error pulling from API - could be invalid lon lat in config or API limit exceeded..")
...
if __name__ == "__main__":
    locations = ["38.8977,-77.0365"]  # White House coordinates (latitude,longitude)
