...
class Wx3Fetcher:
    ...
    def get_weather(self):
        ...
        if response.status_code == 200:
            ...
            condition = " ".join(wx3_info[1:3]).strip()  # Join multi-word conditions like "Heavy drizzle"
            temperature = wx3_info[3].strip()
            feels = wx3_info[4].strip()
            humidity = wx3_info[5].strip()
            wind = wx3_info[6].strip()
            pressure = wx3_info[7].strip()
            precipitation = wx3_info[8].strip()
...
#weather_fetcher = Wx3Fetcher(location)
